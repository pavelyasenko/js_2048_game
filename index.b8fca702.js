...
startButton.addEventListener("click", ()=>{
    if (game.getStatus() === "idle") {
        ...
        updateBoard();
    } else if (game.getStatus() === "playing" || game.getStatus() === "lose" || game.getStatus() === "win") {
        game.restart();
...
class Game {
    ...
    createCleanBord() {
        const board = [];
        for(let i = 0; i < this.size; i++)board.push(Array(this.size).fill(0));
        return board;
    }
    ...
    rotateClockwise(matrix) {
        ...
        for(let r = 0; r < this.size; r++)for(let c = 0; c < this.size; c++)result[c][this.size - 1 - r] = matrix[r][c];
        return result;
    }
    rotateCounterClockwise(matrix) {
        const result = this.createCleanBord();
        for(let r = 0; r < this.size; r++)for(let c = 0; c < this.size; c++)result[this.size - 1 - c][r] = matrix[r][c];
        return result;
    }
    moveLeft() {
    ...
    moveRight() {
        const oldBoard = this.getState();
        const newBoard = this.board.map((row)=>this.slide([
                ...row
            ].reverse()).reverse());
        if (!this.boardsAreEqual(oldBoard, newBoard)) {
            this.board = newBoard;
   ...
   */ restart() {
    ...
    isGameOver() {
        for(let r = 0; r < this.size; r++)for(let c = 0; c < this.size; c++){
            ...
            if (c < this.size - 1 && this.board[r][c] === this.board[r][c + 1] || r < this.size - 1 && this.board[r][c] === this.board[r + 1][c]) return false;
        }
        return true;
    }
